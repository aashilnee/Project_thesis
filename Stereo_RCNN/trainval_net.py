...
def parse_args():
  ...
  parser.add_argument('--epochs', dest='max_epochs',
                      ...
                      default=1200000, type=int)    # TODO ENDRE DENNE

  parser.add_argument('--save_dir', dest='save_dir',
                      help='directory to save models', default="/mnt/raid/digiras_302004685/neural_net/models/",  # TODO
                      type=str)
  ...
  parser.add_argument('--lr_decay_step', dest='lr_decay_step',
                      ...
                      default=2000, type=int)  # TODO ENDRE DENNE
...
class LarvaeDataset(Dataset):
  def __init__(self, file_path): #, transform, convert, im_size=(1848, 3264)):
    ...
    self.im_size = (642, 2560) #(1848, 3264)   # TODO ENDER DENNE?
    #self.transform = transform
    #self.convert = convert
    self.file_path = file_path

    self.data = self.build_data_set()
    print("TEST", len(self.data))
...
# TODO NY
  def build_data_set(self):
    ...
    with open(self.file_path + "labels.csv") as csv_file:
      csv_reader = csv.reader(csv_file, delimiter=';')   # TODO komma deler opp

      #for i, row in enumerate(csv_reader):
        #print("TEST0 ", str(row[0]))
        #print("TEST1 ", str(row[1]))
        #print("TEST2 ", str(row[2]))
        #print("TEST3 ", str(row[3]))
        #print("TEST4 ", str(row[4]))
        #print("TEST5 ", str(row[5]))
        #print("TEST6 ", str(row[6]))
      ...
      for i, row in enumerate(csv_reader):
        if row[5] != '{}' and i > 0:
          ...
          if attributes_dict['name'] == 'roi':
            ...
            img = cv2.imread(self.file_path + row[0])     # TODO HER ER BILDE
            #cv2.imshow('image',img)   # TODO
            #cv2.waitKey(0)      # TODO

            l_img = img[:, :int(img.shape[1] / 2), :]     # TODO BILDE
            r_img = img[:, int(img.shape[1] / 2) - 1:-1, :]  # TODO BILDE (ett bilde)


            #print("opprinnelig l_img: ", len(l_img))
            #print("opprinnelig r_img: ", len(r_img))

            # TODO show whole image, left image, right image
            from matplotlib import pyplot as plt

            #plt.subplot(3, 1, 1), plt.imshow(img, 'gray')
            #plt.title("Whole image")
            #plt.subplot(3, 1, 2), plt.imshow(l_img, 'gray')
            #plt.title("Left image")
            #plt.subplot(3, 1, 3), plt.imshow(r_img, 'gray')
            ...
            #plt.imshow(img, 'gray')
            #plt.show()
            #plt.subplot(1,2,1), plt.imshow(l_img, 'gray')
            #plt.title("Left image")
            #plt.subplot(1, 2, 2), plt.imshow(r_img, 'gray')
            #plt.title("Right image")
            #plt.show()
            ...
            #print("opprinnelig l_rois: ", l_rois)
            #print("opprinnelig r_rois: ", r_rois)

            data_point = self.compose_data(l_img, r_img, l_rois, r_rois)

            #print("restulterende l_rois: ", l_rois)  # TODO størrelsene på rois blir større
            #print("resulterende r_rois: ", r_rois)
  ...
  def compose_data(self, l_img, r_img, l_rois, r_rois):
    ...
    l_img = np.moveaxis(l_img.copy(), -1, 0)  # TODO endrer aksene 1 til venstre. HVORFOR??
    r_img = np.moveaxis(r_img.copy(), -1, 0)

    #cv2.imshow('imagee', l_img)  # TODO
    #cv2.waitKey(0)  # TODO
    ...
    # left and right ROIS
    l_temp = np.zeros([30, 5])                        # TODO 30x5 0
    l_rois[:, 2] = l_rois[:, 0] + l_rois[:, 2]
    l_rois[:, 3] = l_rois[:, 1] + l_rois[:, 3]

    #print("l_roi: ", l_rois)
    #print("l_roisss_2: ", l_rois[:, 2])
...
if __name__ == '__main__':
  ...
  #print("TEST: train size", train_size)

  #print("TEST: imdb", imdb.classes)   # background og Car
  ...
  # dataset path
  train_dir = "data/training_data/" #"/mnt/raid/digiras_302004685/neural_net/training_data/"    # TODO dataset path

  dataset = LarvaeDataset(file_path=train_dir)#, transform=image_transforms['train'], convert=image_transforms['convert']),

  # Dataloader iterators
  dataloader = torch.utils.data.DataLoader(dataset, batch_size=1)   # TODO BATCH_SIZE
  ...
  gt_boxes_left = Variable(torch.FloatTensor([1]).cpu())
  gt_boxes_right = Variable(torch.FloatTensor(1).cpu())
  gt_boxes_merge = Variable(torch.FloatTensor(1).cpu())
  gt_dim_orien = Variable(torch.FloatTensor(1).cpu())
  gt_kpts = Variable(torch.FloatTensor(1).cpu())
  ...
  uncert = Variable(torch.rand(6).cpu(), requires_grad=True)
  ...
  optimizer = torch.optim.Adam(params, lr=lr)#, momentum=cfg.TRAIN.MOMENTUM)    # TODO ADAM OPTIMIZER
...
'''
    ...
    if epoch % 100 == 0:   # TODO lagrer fil hver 100 epok
